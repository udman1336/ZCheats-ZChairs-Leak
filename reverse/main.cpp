...
std::string name = "Z cheat"; // application name. right above the blurred text aka the secret on the licenses tab among other tabs
std::string ownerid = "4CLnrfeqKZ"; // ownerid, found in account settings. click your profile picture on top right of dashboard and then account settings.
std::string secret = "7b47f4bf2cb3c6bfc6be060fed66b90f7f0d63034c7ec8d6516330609a432970"; // app secret, the blurred text on licenses tab and other tabs
...

{
	system("color 3");
	for (int i = 0; i <= 0; i++) {
		std::cout << "\n Zchairs paid fortnite external chair \n";
	}
	Sleep(2500);
	system("cls");
	...
	system("color 3");
	std::cout << termcolor::green << slowly_printing_string{ "\n\n Loading External Cheat...", 50 };	Sleep(3000);
	std::cout << termcolor::white;
	...
	SetConsoleTitleA(random_string(30).c_str());
	system("color 3");
	std::cout << termcolor::green << "\n\n  Load/Open Fortnite and press F7 in lobby";
	std::cout << termcolor::white;
	while (true)
	{
		Sleep(10);
...
void xInitD3d()
{
	...
	style->WindowMinSize = ImVec2(70, 33); // Reduced height by 30%
	...
	// Updated color values with red shades
	style->Colors[ImGuiCol_WindowBg] = ImColor(33, 26, 46);
	style->Colors[ImGuiCol_Border] = ImColor(33, 26, 46);
	style->Colors[ImGuiCol_FrameBg] = ImColor(119, 0, 255, 255);
	style->Colors[ImGuiCol_FrameBgHovered] = ImColor(119, 0, 255, 255);
	style->Colors[ImGuiCol_FrameBgActive] = ImColor(160, 70, 220, 255);
	...
	style->Colors[ImGuiCol_Button] = ImColor(119, 0, 255, 255);
	style->Colors[ImGuiCol_ButtonHovered] = ImColor(119, 0, 255, 255);
	style->Colors[ImGuiCol_ButtonActive] = ImColor(160, 70, 220, 255);
	style->Colors[ImGuiCol_Separator] = ImColor(247, 3, 89, 255);
	style->Colors[ImGuiCol_TitleBg] = ImColor(33, 26, 46);
	style->Colors[ImGuiCol_TitleBgActive] = ImColor(33, 26, 46);
	style->Colors[ImGuiCol_TitleBgCollapsed] = ImColor(33, 26, 46);
...
for (uint32_t i = 0; i < Num; i++)
{
   ...
   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
	...
	if (watermark) {
		...
		ImGui::GetOverlayDrawList()->AddText(ImVec2(8, 2), IM_COL32(150, 10, 10, 255), dist);

		// Second line with darker red color
		sprintf_s(dist, " Zchairs fortnite external \n", ImGui::GetIO().Framerate);
		ImGui::GetOverlayDrawList()->AddText(ImVec2(8, 2), IM_COL32(150, 10, 10, 255), dist);

		// Third line with darker red color
		sprintf_s(dist, " V2\n", ImGui::GetIO().Framerate);
		ImGui::GetOverlayDrawList()->AddText(ImVec2(8, 15), IM_COL32(150, 10, 10, 255), dist);
	...
	if (distance < VisDist)
	{
		...
		if (Esp)
		{
			if (crosshairEnabled)
			{
				...
				ImVec2 crosshairLeft(center.x - crosshairSize, center.y);
				ImVec2 crosshairRight(center.x + crosshairSize, center.y);
				ImVec2 crosshairTop(center.x, center.y - crosshairSize);
				ImVec2 crosshairBottom(center.x, center.y + crosshairSize);

				// Fixed red color (RGBA: 255, 0, 0, 255)
				ImU32 crosshairColorU32 = IM_COL32(255, 0, 0, 255);

				// Make sure you have the correct ImGui::GetOverlayDrawList() call here.
				ImGui::GetOverlayDrawList()->AddLine(crosshairLeft, crosshairRight, crosshairColorU32, 2.0f);
				ImGui::GetOverlayDrawList()->AddLine(crosshairTop, crosshairBottom, crosshairColorU32, 2.0f);
			}
			...
			if (skeletonesp)
			{
				...
				Vector3 vLeftFootOut = ProjectWorldToScreen(vLeftFoot);
				Vector3 vRightFootOut = ProjectWorldToScreen(vRightFoot);
				DrawLine(vHeadBoneOut.x, vHeadBoneOut.y, vNeckOut.x, vNeckOut.y, &Col.red, 3.f);
				DrawLine(vHipOut.x, vHipOut.y, vNeckOut.x, vNeckOut.y, &Col.red, 3.f);
				DrawLine(vUpperArmLeftOut.x, vUpperArmLeftOut.y, vNeckOut.x, vNeckOut.y, &Col.red, 3.f);
				DrawLine(vUpperArmRightOut.x, vUpperArmRightOut.y, vNeckOut.x, vNeckOut.y, &Col.red, 3.f);
				DrawLine(vLeftHandOut.x, vLeftHandOut.y, vUpperArmLeftOut.x, vUpperArmLeftOut.y, &Col.red, 3.f);
				DrawLine(vRightHandOut.x, vRightHandOut.y, vUpperArmRightOut.x, vUpperArmRightOut.y, &Col.red, 3.f);
				DrawLine(vLeftHandOut.x, vLeftHandOut.y, vLeftHandOut1.x, vLeftHandOut1.y, &Col.red, 3.f);
				DrawLine(vRightHandOut.x, vRightHandOut.y, vRightHandOut1.x, vRightHandOut1.y, &Col.red, 3.f);
				DrawLine(vLeftThighOut.x, vLeftThighOut.y, vHipOut.x, vHipOut.y, &Col.red, 3.f);
				DrawLine(vRightThighOut.x, vRightThighOut.y, vHipOut.x, vHipOut.y, &Col.red, 3.f);
				DrawLine(vLeftCalfOut.x, vLeftCalfOut.y, vLeftThighOut.x, vLeftThighOut.y, &Col.red, 3.f);
				DrawLine(vRightCalfOut.x, vRightCalfOut.y, vRightThighOut.x, vRightThighOut.y, &Col.red, 3.f);
				DrawLine(vLeftFootOut.x, vLeftFootOut.y, vLeftCalfOut.x, vLeftCalfOut.y, &Col.red, 3.f);
				DrawLine(vRightFootOut.x, vRightFootOut.y, vRightCalfOut.x, vRightCalfOut.y, &Col.red, 3.f);
			}
			...
			if (fillbox)
			{
				DrawCorneredBox(Headbox.x - (CornerWidth / 2), Headbox.y, CornerWidth, CornerHeight, IM_COL32(255, 108, 0, 255), 2.5);
	...
	bool showCrosshair = true; // Variable to control crosshair visibility
	if (exploits) {
		if (fpsCounter)
		{
			char dist[64];
			XorS(frame, "%.1f Fps | Zchairs fn \n");
		...
		{
			if (isVisible)
				DrawNormalBox(Headbox.x - (CornerWidth / 2), Headbox.y, CornerWidth, CornerHeight, 2.5f, ImColor(0, 8, 255, 255));
			else
				DrawNormalBox(Headbox.x - (CornerWidth / 2), Headbox.y, CornerWidth, CornerHeight, 2.5f, ImColor(255, 0, 38, 125));
		}
	}
...
void render() {
	...
	if (ShowMenu)
	{
		...
		bool showAimbot = true;
		bool showVisuals = false;
		bool showExploits = false;
		bool showInfo = false;

		ImGui::SetNextWindowSize({ 510, 440 });
		ImGui::Begin("Zchairs paid fn external | By Serfux", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);

		ImVec2 windowPos = ImGui::GetWindowPos();
		ImVec2 windowSize = ImGui::GetWindowSize();

		if (ImGui::Button("Discord Server", ImVec2(100, 30)))
		{
			exit(0);
		}
		...
		if (showAimbot)
		{
			ImGui::BeginChild("##ChildIdentifier1", ImVec2(420, 480), false);
			{
				ImGui::Checkbox("Aimbot", &Aimbot);
				ImGui::Text("Aimbot Bone");
				ImGui::Combo("              ", &hitboxpos, hitboxes, sizeof(hitboxes) / sizeof(*hitboxes));
				ImGui::Text("Fov Type");
				...
				ImGui::Spacing();
				ImGui::SliderFloat("Aim Smoothness", &smooth, 1, 10);
				ImGui::SliderFloat("Aimbot FOV Size", &AimFOV, 50.f, 800.f);
				ImGui::Spacing();
				ImGui::Text("Aim Key");
		...
		if (showVisuals)
		{
			ImGui::BeginChild("##ChildIdentifier2", ImVec2(390, 500), false);
			{
				...
				ImGui::Checkbox("Filled Boxes Esp", &fillbox);
				ImGui::Checkbox("3D Box Esp", &ThreeDBoxEsp);
				ImGui::Checkbox("Crosshair", &crosshairEnabled);
				ImGui::SameLine();
		...
		if (showExploits)
		{
			ImGui::BeginChild("##ChildIdentifier3", ImVec2(750, 500), false);
			{
				ImGui::Text("Exploits:");
				ImGui::Checkbox("Enable Exploits", &exploits);
				ImGui::Checkbox("FPS Counter", &fpsCounter);
				ImGui::Checkbox("Stream Proof", &WindowStreamProof);
				ImGui::Spacing();
				ImGui::Spacing();
			}
		...
		if (showInfo)
		{
			ImGui::BeginChild("##ChildIdentifier4", ImVec2(400, 300), false);
			{
				...
				ImGui::Text("[ Fully Madey by Serfux ]");
				ImGui::Text("[ Fully ud on both anticheats ]");
				ImGui::Text("[ Working on Windows 10 / 11 ]");
				ImGui::Text("[ If the menu or cheat has any issues -");
				ImGui::Text("please report it to the server! ]");
			}
			ImGui::EndChild();
		}
		ImGui::SetCursorPosY(ImGui::GetWindowHeight() - 40); // Adjust the Y position to place the button where you want
		if (ImGui::Button("Exit Cheat", ImVec2(100, 30)))
		{
